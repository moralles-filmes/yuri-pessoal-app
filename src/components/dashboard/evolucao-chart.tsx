...
/**
 * Fachada de carregamento sob demanda de `evolucao-chart-impl` (P3 da auditoria de performance, F-003).
 *
...
type Props = React.ComponentProps<typeof import("./evolucao-chart-impl").EvolucaoChart>;
...
const LazyEvolucaoChart = dynamic(() => import("./evolucao-chart-impl").then((m) => m.EvolucaoChart), {
  ssr: false,
  loading: () => <ChartSkeleton />,
});

export function EvolucaoChart(props: Props) {
  return (
    <div style={chartSkeletonHeightVar(280)}>
      <LazyEvolucaoChart {...props} />
    </div>
  );
}
