-- ══════════════════════════════════════════════════════════════════════════════════════
-- Fase 18-F · Bloco 4 — EXPERIÊNCIAS · janela de dedupe do clique duplo
--
-- ⛔ NENHUMA TABELA, NENHUMA COLUNA, NENHUM ÍNDICE. Só `create or replace` da função de
-- admissão, com UM bloco a mais.
--
-- O QUE ESTA MIGRATION CORRIGE (achado P2 da auditoria do bloco, 2026-09-22):
--
-- O atalho de panorama SEMPRE gasta dinheiro — ao contrário de uma proposta do Approval
-- Engine, que expira sem efeito. O único freio era o estado `enviando` do cliente. O advisory
-- lock SERIALIZA duas chamadas simultâneas, mas não as DEDUPLICA: a segunda espera a primeira
-- commitar, lê a reserva dela e, se couber no orçamento, cria uma SEGUNDA conversa, um
-- SEGUNDO run e um SEGUNDO gasto — pelo mesmo panorama.
--
-- ⚠️ POR QUE UMA JANELA, E NÃO "RECUSAR ENQUANTO HOUVER RUN ABERTO":
--
-- Recusar por run aberto sem prazo travaria o dono por até 5 MINUTOS (a lease de
-- `ai_reconcile_abandoned_runs`) depois de ele fechar a aba no meio de um panorama. Trocaria
-- um aborrecimento de custo — que o orçamento já limita — por uma funcionalidade bloqueada.
-- A janela mata o clique duplo (dois pedidos em menos de um segundo) e devolve o botão em
-- seguida. É a invariante 68 aplicada aqui: DEDUPLICAR ANTES DE GASTAR.
--
-- ⛔ E ela é conferida DEPOIS do advisory lock, por isso não tem corrida: as admissões do
-- mesmo dono já entram uma de cada vez, então a segunda enxerga a linha que a primeira
-- gravou.
--
-- Idempotente.
-- ══════════════════════════════════════════════════════════════════════════════════════
...
-- ══════════════════════════════════════════════════════════════════════════════════════
-- ai_begin_experience_run — admissão atômica de um run de EXPERIÊNCIA
--
-- O corpo abaixo é o de `20260922100000_ai_experiencias.sql` POR INTEIRO, mais o passo 5b.
-- Repetir a função inteira é o que `create or replace` exige, e é também o que mantém o
-- arquivo legível sozinho: uma migration que só dissesse "acrescente este `if`" obrigaria
-- quem a lesse a reconstruir a função de cabeça.
-- ══════════════════════════════════════════════════════════════════════════════════════
--
...
--   passo 5b               → janela de dedupe do clique duplo (não existe no chat: lá o
--                            texto digitado distingue os pedidos, aqui os dois são iguais)
--
...
declare
  ...
  -- A janela do clique duplo. Curta de propósito: ela existe para o gesto, não para o uso.
  -- Um dono que queira MESMO dois panoramas iguais espera 15 segundos e os tem.
  c_janela         constant interval := interval '15 seconds';
...
begin
  ...
  -- ── 5b. CLIQUE DUPLO — dedupe por JANELA, não por run aberto ────────────────────────
  --
  -- ⛔ AS DUAS CONDIÇÕES SÃO NECESSÁRIAS, e cada uma desarma um defeito da outra:
  --
  --   `status in ('reserved','streaming')` sozinho travaria por 5 minutos (a lease).
  --   `created_at > agora - 15s` sozinho recusaria o RETRY legítimo de um panorama que
  --   acabou de falhar — e falhar rápido é o caso mais comum de querer clicar de novo.
  --
  -- ⚠️ `created_at`, e não `started_at`: é a coluna que as outras janelas desta função usam
  -- (rate limit, orçamento do dia e do mês). Dentro de uma transação as duas valem o mesmo
  -- instante, então a escolha é de LEGIBILIDADE — quem comparar as janelas não deve ter de
  -- descobrir por que só esta olha outra coluna.
  --
  -- Juntas: o segundo clique do mesmo gesto é recusado, e um run que fechou (por falha,
  -- cancelamento ou sucesso) devolve o botão na hora.
  --
  -- ⚠️ `agent_id` discrimina a EXPERIÊNCIA: "Planejar meu dia" não bloqueia "Encerrar meu
  -- dia". São panoramas diferentes, e pedir os dois em sequência é uso normal.
  if exists (
    select 1
      from public.ai_runs r
     where r.user_id = v_user
       and r.kind = 'experience'
       and r.agent_id = v_agent
       and r.status in ('reserved', 'streaming')
       and r.created_at > v_now - c_janela
  ) then
    raise exception 'AI_EXPERIENCE_JUST_STARTED' using errcode = 'P0001';
  end if;
...
comment on function public.ai_begin_experience_run(text, text, text, text, text, numeric, text, integer) is
  ...
  'catálogo, nunca texto do cliente. O passo 5b deduplica o clique duplo por uma janela de 15s, '
  'sobre runs de experiência ainda abertos DESTE mesmo panorama.';
